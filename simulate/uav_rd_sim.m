...
radar.S_slope_in_mhzus = 15.0148e12;    % Chirp slope S (MHz/us)
...
t_slow = (0:numChirps-1) * radar.T_chirp;  % slow-time (per chirp)
...
N_scatterer_per_blade = uav.N_scat_per_blade;

% total rotor scatterers
N_rotor_scatterer = N_rotor * N_blade * N_scatterer_per_blade;

% Allocate arrays for rotor scatterer parameters
r_list = zeros(N_rotor_scatterer,1);  % radius along blade
phi0_list = zeros(N_rotor_scatterer,1);  % initial rotor angle
omega_list= zeros(N_rotor_scatterer,1);  % angular velocities (rad/s)
...
for s = 1:N_rotor_scatterer
    ...
    R_md(s_idx, :) = - r_s * cos(beta) * cos(omega_s * t_slow + phi0_s); % what if I don't add minus
end
...
R_body_mat = repmat(R_body, N_scat_total, 1);  % S x numChirps
...
S_range = fft(S_win, N_fft_range, 2);           % numChirps x N_fft_range
